...
function export_utilisation_station_JSON() {
    ...
    while ($row = mysqli_fetch_assoc($resultat)) {
        $requete = "SELECT horaire_semaine,SUM(valeur) AS utilisation FROM Trajet WHERE station1=".$row['station1']." OR station2=".$row['station1']." GROUP BY horaire_semaine";
    ...
    $requete = "SELECT horaire_semaine,SUM(valeur) AS utilisation FROM Trajet GROUP BY horaire_semaine";
    $result = mysqli_query($connexion, $requete);
    // On met toutes les données d'utilisation de la station dans un tableau, que l'on convertira en JSON après
